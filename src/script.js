...
let getName = (blob) => {
  ...
  $.ajax(settings).done(async (response) => {
    ...
    if (name) {
      ...
      await sleep(2000);
      textToSpeech("Your order is confirmed.");
      document.getElementById("order-box").style.display = "block"
      document.getElementById("uname-box").style.display = "none"
      document.getElementById("cup-name").innerHTML = name;
      getUserRatings();
      return name;
    }
    else {
...
let displayFeedback = async (arr) => {
  if (!finalRating) {
    ...
    textToSpeech("Thank you for your feedback. Have a wonderful day!")
    finalRating = 0;
    flag='order'
    ratingArr = []
    await sleep(10000);
...
async function predict() {
  ...
  if (flag == 'order') {
    ...
    for (let i = 0; i < maxPredictions; i++) {
      ...
      if (confidence >= 0.9 && !speechContainer.innerHTML.includes(translation) && !orderedRight) {
